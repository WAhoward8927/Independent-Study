...
int power_measurement_delay = 3000;
...
const String GOOGLE_USERNAME = "mymikudiva";
const String GOOGLE_PASSWORD = "temboocsie01";
...
void loop() {
  ...
  if (client) {
    ...
    if (power_measurement_cycles > power_measurement_cycles_max) {
     ...
     interval = millis() - last_measurement;
     last_measurement = millis();
    ...
    runAppendRow(interval,effective_power);
...
void runAppendRow(int interval, float Power) {
  ...
  AppendRowChoreo.addInput("SpreadsheetTitle","Power");

  AppendRowChoreo.addInput("RefreshToken","ya29.GltjBIbh6Omc5b4wWQSmMUw4QhAYBCU6AjNnQOILdbisvFK5lX97vsDdHNvrT7nHSry5KN4QcZL9hnl_bLxbGlQvsGlszwozuochMJyePCesS44jlyTEk625_Iuk");
  
  AppendRowChoreo.addInput("ClientSecret","-bG2P-r04dN8i2XBS0Qlc9tw");
  
  AppendRowChoreo.addInput("ClientID","1013391984596-iqurd7emb9kjhndomlb1kctkpkhtbk8v.apps.googleusercontent.com");
  ...
  // Format data
  String data = "";
  data = data + timeString + "," + String(interval) + "," + String(Power);
